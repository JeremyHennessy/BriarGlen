...
try{
  for(const vp of views){
    ...
    await page.waitForFunction(()=>Boolean(window.__BRIAR_GLENDebug?.getSourceArt47State&&window.__BRIAR_GLENDebug?.getSourceArt47LayerGuard&&window.__BRIAR_GLENDebug?.getSourceGround47State&&window.__BRIAR_GLENDebug?.getGroundV2State),{timeout:8000});
    await page.waitForFunction(()=>window.__BRIAR_GLENDebug.getSourceArt47State().ready&&window.__BRIAR_GLENDebug.getSourceGround47State().ready&&window.__BRIAR_GLENDebug.getGroundV2State().ready,{timeout:10000});
    let s=await page.evaluate(()=>({
      source:window.__BRIAR_GLENDebug.getSourceArt47State(),
      guard:window.__BRIAR_GLENDebug.getSourceArt47LayerGuard(),
      groundOwner:window.__BRIAR_GLENDebug.getSourceGround47State(),
    ...
    if(!s.groundOwner.enabled||s.groundOwner.failed||s.groundOwner.physicalTileCount!==21)throw new Error(`${vp.name}: authoritative source ground inactive ${JSON.stringify(s.groundOwner)}`);
    if(s.ground.sourceMode!=='build47-physical-tiles'||s.ground.physicalTileCount!==21||!s.ground.enabled)throw new Error(`${vp.name}: Ground V2 bridge not active ${JSON.stringify(s.ground)}`);
    ...
    for(const [name,x,y] of regions){
      ...
      s=await page.evaluate(()=>({source:window.__BRIAR_GLENDebug.getSourceArt47State(),groundOwner:window.__BRIAR_GLENDebug.getSourceGround47State(),ground:window.__BRIAR_GLENDebug.getGroundV2State()}));
      if(s.groundOwner.frameChunks<1||s.groundOwner.frameCells<64||s.ground.frameChunks<1||s.ground.frameCells<64)throw new Error(`${vp.name}/${name}: authoritative terrain cache not drawing ${JSON.stringify(s)}`);
    ...
    console.log(`PASS ${vp.name}: authoritative physical terrain + scale-correct source art, ${regionsDrawn}/7 regions, legacy overlap layers suppressed`);
    await context.close();
  }
  ...
  await page.waitForFunction(()=>Boolean(window.__BRIAR_GLENDebug?.getSourceArt47State&&window.__BRIAR_GLENDebug?.getSourceArt47LayerGuard&&window.__BRIAR_GLENDebug?.getSourceGround47State&&window.__BRIAR_GLENDebug?.getGroundV2State),{timeout:8000});
  const off=await page.evaluate(()=>({source:window.__BRIAR_GLENDebug.getSourceArt47State(),guard:window.__BRIAR_GLENDebug.getSourceArt47LayerGuard(),groundOwner:window.__BRIAR_GLENDebug.getSourceGround47State(),ground:window.__BRIAR_GLENDebug.getGroundV2State()}));
  ...
  if(off.groundOwner.enabled||off.groundOwner.requested)throw new Error(`sourceArt47=0 did not disable authoritative source ground ${JSON.stringify(off.groundOwner)}`);
