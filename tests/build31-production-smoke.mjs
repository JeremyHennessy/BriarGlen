...
try{
  for(const vp of viewports){
    ...
    await page.evaluate(()=>{const d=window.__BRIAR_GLENDebug;d.teleport(-620,20);});
    await sleep(500);
    let town=await page.evaluate(()=>window.__BRIAR_GLENDebug.getCharacterArtState());
    if(town.npcDraws<1)throw new Error(`${vp.name}: Build 31 revised town cast did not draw ${JSON.stringify(town)}`);
    ...
    if(Object.keys(state.cast.loadedCharacterAssets||{}).length!==13||Object.values(state.cast.loadedCharacterAssets||{}).some(v=>!v.loaded))throw new Error(`${vp.name}: revised Living Cast source family incomplete ${JSON.stringify(state.cast.loadedCharacterAssets)}`);
    if(state.cast.playerDraws<1||state.cast.enemyDraws<1||state.cast.npcDraws<1||state.cast.replacements.player_sword<1||state.cast.replacements.wolf<1)throw new Error(`${vp.name}: revised Living Cast production draws missing ${JSON.stringify(state.cast)}`);
    ...
    console.log(`PASS ${vp.name}: Build 31 revised Living Cast is production default`);
    await context.close();
  }
