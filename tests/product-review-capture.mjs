...
async function setFullState(page){
  await page.evaluate(()=>{
    ...
    d.setProgress({
      step:5,contractComplete:true,patrolComplete:true,bossDefeated:true,shortcutUnlocked:true,
      groveDiscovered:true,grovekeeperDefeated:true,groveCacheClaimed:true,reinforcedPickaxe:true,
      temperedSword:true,briarstringBow:true,moonrootStaff:true,gearVest:true,gearCharm:true,
      groveRelicOwned:true,wardenBootsOwned:true,fenDiscovered:true,fenCrossingOpened:true,
      fenWardenDefeated:true,fenCacheClaimed:true,stonepinePassOpened:true,stonepineDiscovered:true,
      stonepineBossDefeated:true,stonepineCacheClaimed:false,boardContractsCompleted:2,
      boardContractCounts:{copper_order:1,briar_cull:1},activeBoardContract:null,
    });
    d.setPlayer({reinforced:true,coins:1000,hp:100,maxHp:125,invuln:0});
    d.setInventory({herb:7,mooncap:4,ore:8,iron:5,hide:6,tusk:1,tonic:3,oil:2,mossglass:3,resin:4,binding:1});
  });
  await sleep(180);
}
...
async function captureEstablished(vp){
  ...
  await page.evaluate(()=>{
    const d=window.__BRIAR_GLENDebug;
    d.setProgress({groveDiscovered:true,shortcutUnlocked:true,fenDiscovered:true,fenCrossingOpened:true,fenWardenDefeated:true,fenCacheClaimed:true,stonepinePassOpened:true,stonepineDiscovered:true,stonepineBossDefeated:true,stonepineCacheClaimed:false,boardContractsCompleted:3,contractComplete:true,patrolComplete:true,grovekeeperDefeated:true,groveCacheClaimed:true,reinforcedPickaxe:true,temperedSword:true,briarstringBow:true,moonrootStaff:true,gearVest:true,gearCharm:true,groveRelicOwned:true,wardenBootsOwned:true});
    d.openMap();
  });
  ...
  await page.evaluate(()=>{const d=window.__BRIAR_GLENDebug;d.setThreat('quarrysentinel',{hp:290,dead:false,hurt:0});const s=d.getStonepineState();const e=s.enemies.find(v=>v.type==='quarrysentinel');if(e)d.teleport(e.x-170,e.y);else d.teleport(3100,-1760);});
  ...
  await page.evaluate(()=>{localStorage.removeItem('briar-glen-vertical-slice-complete-v1');const d=window.__BRIAR_GLENDebug;d.setProgress({stonepinePassOpened:true,stonepineDiscovered:true,stonepineBossDefeated:true,stonepineCacheClaimed:false});d.teleport(3255,-1900);});
...
try{
  for(const vp of viewports){await captureOnboarding(vp);await captureEstablished(vp);}
  fs.writeFileSync(path.join(root,'manifest.json'),JSON.stringify({generatedAt:new Date().toISOString(),baseline:'Build 30 production / 69fe5815f9a569c2ca4a5b4c498a5368d49878fe',screenshots:manifest},null,2));
