...
try{
  for(const vp of views){
    ...
    if(v.familyId!=='briar-glen-art-v1'||v.recipeId!=='briar-glen-art-v1-painted-family-v1'||v.version!=='art-v1-vegetation-preview-v3-clean-atlas')throw new Error(`${vp.name}: family/recipe/version drift ${JSON.stringify(v)}`);
    if(v.atlasPath!=='assets/art-v1/vegetation/vegetation-atlas-v3.webp'||v.atlasWidth!==512||v.atlasHeight!==384||v.roleCount!==9||v.sourceMasterCount!==9)throw new Error(`${vp.name}: vegetation asset contract drift ${JSON.stringify(v)}`);
  ...
  await atlasPage.screenshot({path:'artifacts/art-v1-vegetation-atlas-v3-browser.png'});await atlasContext.close();
