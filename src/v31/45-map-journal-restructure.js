(() => {
  ...
  function economy(){const response=progress.wardenResponse32||{},record=window.__BRIAR_GLEN_SPECIALIST_ECONOMY38?.record?.(response.completedBySource||{});return[['Board jobs',progress.boardContractsCompleted||0],['Rowan market',progress.market14?.purchases||progress.stonepineSupply?.purchases? 'Engaged':'Not engaged'],['Warden responses',response.fulfilled||0],['Repeat clients',record?.summary||'No repeat clients yet'],['Stonepine patrols',progress.stonepineWork?.completed||0]];}
  function render(){
