(() => {
  ...
  const state={version:'build47-visual-rebuild-v4',requested,sourceSpritesRequested,historicalProof,ready:!requested,failed:false,frameDraws:0,totalDraws:0,terrainMode:'physical-ground-v2',sources:{},draws:{},baseline:{objects:worldObjects.length,resources:resources.length,enemies:enemies.length}};
  const generatedAtlas=new Image();generatedAtlas.decoding='async';
  const external={cottage:{src:'assets/v24/cottage-authored.webp',width:96,height:96,anchor:.84},tree_deciduous:{src:'assets/v24/tall-tree-authored.webp',width:72,height:72,anchor:.91},tree_pine:{src:'assets/v24/pine-tree-authored.webp',width:76,height:76,anchor:.92},rock:{src:'assets/v47/rock-cluster.svg',width:58,height:51,anchor:.88},ore:{src:'assets/v47/ore-node.svg',width:47,height:42,anchor:.90},utility:{src:'assets/v47/utility-crates.svg',width:52,height:46,anchor:.91},sign:{src:'assets/v47/sign-lantern.svg',width:42,height:52,anchor:.91}};
  ...
  for(const[name,def]of Object.entries(external)){const image=new Image();image.decoding='async';image.onload=()=>{images[name]=rasterize(image,def);state.sources[name]=def.src;done();};image.onerror=()=>{failures.push(def.src);done();};image.src=`${def.src}?v=47r4`;}
  ...
  drawObject=function build47ScaleCorrectedObject(o){
    ...
    else if(o.type==='quarryRock'){if(drawExternal('rock',o,{scale:.82,alpha:.94}))return;}
    else if(o.type==='denRock'){if(drawExternal('rock',o,{scale:.84,alpha:.92}))return;}
    else if(['ruin','fenRuin','stonepineRuin'].includes(o.type)){if(drawExternal('rock',o,{scale:.62,alpha:.90}))return;}
    else if(o.type==='deadTree'){if(drawGenerated('stump',o,{scale:.52}))return;}
    else if(o.type==='bush'){if(drawGenerated('bush',o,{scale:.58}))return;}
    else if(o.type==='garden'){if(drawGenerated('flower_clump',o,{scale:.58}))return;}
    else if(o.type==='fence'){if(drawGenerated('fence',o,{scale:.62}))return;}
    else if(o.type==='lamp'){if(drawGenerated('lamp',o,{scale:.62}))return;}
    else if(o.type==='ember'){if(drawGenerated('campfire',o,{scale:.42,alpha:.92}))return;}
    else if(['groveSign','fenSign','stonepineSign'].includes(o.type)){if(drawExternal('sign',o,{scale:.72}))return;}
  ...
  drawResource=function build47ScaleCorrectedResource(r){
    ...
    if(r.type==='herb'&&drawGenerated('herb_clump',r,{scale:.58}))return;
    if(r.type==='mooncap'&&drawGenerated('mooncap_clump',r,{scale:.58}))return;
    if(r.type==='resin'&&drawGenerated('log_pile',r,{scale:.42}))return;
  ...
  debug.isSourceArt47Enabled=enabled;debug.getSourceArt47State=()=>({version:state.version,requested:state.requested,sourceSpritesRequested:state.sourceSpritesRequested,historicalProof:state.historicalProof,enabled:enabled(),ready:state.ready,failed:state.failed,failure:state.failure||'',frameDraws:state.frameDraws,totalDraws:state.totalDraws,terrainMode:state.terrainMode,sources:{...state.sources},draws:{...state.draws},baseline:{...state.baseline},current:{objects:worldObjects.length,resources:resources.length,enemies:enemies.length}});
})();