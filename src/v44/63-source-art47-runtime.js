(() => {
  ...
  function drawExternal(name,o,{scale=1,alpha=1,filter='',flip=false,dx=0,dy=0}={}){const def=external[name],image=images[name];if(!def||!image)return false;const p=worldToScreen(o.x,o.y),z=camera.zoom*scale*entityScale(o),w=def.width*z,h=def.height*z;if(!visible(p,w,h))return true;ctx.save();ctx.globalAlpha=alpha;ctx.imageSmoothingEnabled=true;ctx.imageSmoothingQuality='high';if(filter)ctx.filter=filter;ctx.translate(p.x+dx*camera.zoom,p.y+dy*camera.zoom);if(flip)ctx.scale(-1,1);ctx.drawImage(image,-w/2,-h*def.anchor,w,h);ctx.restore();record(name);return true;}
  const serviceScale={tavern:.72,forge:.74,alchemy:.72,market:.74,well:.76};
  function drawGenerated(name,o,{scale=1,alpha=1,flip=false,dx=0,dy=0}={}){const f=pack.sprites[name];if(!f)return false;const p=worldToScreen(o.x,o.y),z=camera.zoom*scale*entityScale(o),w=f.width*z,h=f.height*z;if(!visible(p,w,h))return true;ctx.save();ctx.globalAlpha=alpha;ctx.imageSmoothingEnabled=true;ctx.imageSmoothingQuality='high';ctx.translate(p.x+dx*camera.zoom,p.y+dy*camera.zoom);if(flip)ctx.scale(-1,1);ctx.drawImage(generatedAtlas,f.sx,f.sy,f.sw,f.sh,-w/2,-h*f.anchor,w,h);ctx.restore();record(name);return true;}
  ...
  drawObject=function build47ScaleCorrectedObject(o){
    ...
    else if(o.type==='fenTree'){if(drawExternal('tree_deciduous',o,{scale:.86,filter:'hue-rotate(24deg) saturate(.55) brightness(.74)'}))return;}
    else if(o.type==='stonepineTree'){if(drawExternal('tree_pine',o,{scale:.90,filter:'saturate(.72) brightness(.80)'}))return;}
    else if(o.type==='rock'){if(drawExternal('rock',o,{scale:.94}))return;}
    else if(o.type==='quarryRock'){if(drawExternal('rock',o,{scale:.92,filter:'saturate(.60) brightness(.82)'}))return;}
    else if(o.type==='denRock'){if(drawExternal('rock',o,{scale:.94,filter:'saturate(.50) brightness(.64)'}))return;}
