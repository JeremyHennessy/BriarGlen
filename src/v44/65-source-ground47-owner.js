(() => {
  ...
  const state={version:'build47-ground-owner-v1',requested,ready:!requested,failed:false,cacheBuilds:0,cacheHits:0,cacheMisses:0,evictions:0,frameChunks:0,frameCells:0,activeCache:0,maxCache:CACHE_LIMIT,lastDecal:'',physicalTileCount:requested?21:0,sourceMode:requested?'build47-physical-tiles':'build46-procedural'};
  if(requested){atlas.onload=()=>{state.ready=true;};atlas.onerror=()=>{state.failed=true;state.ready=false;};atlas.src='assets/v47/terrain-materials.svg?v=47-owner';}
  ...
  function drawCell(c,region,gx,gy,wx,wy,dx,dy){const row=rows[region],h=hash(gx,gy,71),ox=crop(h),oy=crop(h>>>5);c.drawImage(atlas,ox,row*256+oy,128,128,dx,dy,CELL_PX,CELL_PX);const s=strength(region,wx,wy);if(s>.06){const h2=hash(gx,gy,113),fx=crop(h2),fy=crop(h2>>>5);c.globalAlpha=.22+s*.48;c.drawImage(atlas,256+fx,row*256+fy,128,128,dx,dy,CELL_PX,CELL_PX);c.globalAlpha=1;}const density={village:17,meadow:13,grove:12,fen:14,copper:15,stonepine:13,den:18}[region];if(s<.22&&hash(gx,gy,151)%density===0){const size=14+(hash(gx,gy,193)%8),pad=(CELL_PX-size)/2;c.globalAlpha=.55;c.drawImage(atlas,512,row*256,256,256,dx+pad,dy+pad,size,size);c.globalAlpha=1;state.lastDecal=`build47-${region}`;}}
  ...
  function drawOwnedGround(){const region=regionAt(player.x,player.y);ctx.fillStyle=bg[region]||'#465047';ctx.fillRect(0,0,viewport.w,viewport.h);state.frameChunks=0;state.frameCells=0;const minCx=Math.floor(BOUNDS.minX/CHUNK_WORLD),maxCx=Math.floor(BOUNDS.maxX/CHUNK_WORLD),minCy=Math.floor(BOUNDS.minY/CHUNK_WORLD),maxCy=Math.floor(BOUNDS.maxY/CHUNK_WORLD),unit=PIXEL_WORLD;for(let cy=minCy;cy<=maxCy;cy++)for(let cx=minCx;cx<=maxCx;cx++){if(!visible(cx,cy))continue;const entry=chunk(cx,cy),p=worldToScreen(cx*CHUNK_WORLD,cy*CHUNK_WORLD);ctx.save();ctx.translate(p.x,p.y);ctx.transform(.78*camera.zoom*unit,.39*camera.zoom*unit,-.78*camera.zoom*unit,.39*camera.zoom*unit,0,0);ctx.drawImage(entry.canvas,0,0);ctx.restore();state.frameChunks++;state.frameCells+=64;}}
