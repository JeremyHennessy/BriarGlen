(() => {
  ...
  const anchors={village:{x:-610,y:0,sx:520,sy:900,bias:1.05},meadow:{x:260,y:0,sx:510,sy:760,bias:1.02},grove:{x:480,y:-790,sx:560,sy:500,bias:1.30},fen:{x:1500,y:-1650,sx:760,sy:620,bias:1.38},copper:{x:1050,y:0,sx:540,sy:760,bias:1.05},stonepine:{x:2800,y:-1520,sx:850,sy:680,bias:1.38},den:{x:1870,y:0,sx:590,sy:760,bias:1.08}};
  const rows=Object.fromEntries(regions.map((r,i)=>[r,i]));
  const atlas=new Image();atlas.decoding='async';
  const state={version:'build47-ground-owner-v4',requested,ready:!requested,failed:false,cacheBuilds:0,cacheHits:0,cacheMisses:0,evictions:0,frameChunks:0,frameCells:0,activeCache:0,maxCache:CACHE_LIMIT,lastDecal:'',physicalTileCount:requested?21:0,sourceMode:requested?'build47-physical-tiles':'build46-procedural',materialPrimary:'',materialSecondary:'',materialMix:0};
  if(requested){atlas.onload=()=>{state.ready=true;};atlas.onerror=()=>{state.failed=true;state.ready=false;};atlas.src='assets/v47/terrain-materials.svg?v=47-owner4';}
  ...
  function materialMixAt(x,y){const n=boundaryNoise(x,y),out=[];for(const region of regions){const a=anchors[region],dx=(x-a.x)/(a.sx*(1+n*.16)),dy=(y-a.y)/(a.sy*(1-n*.12)),w=Math.exp(-.5*(dx*dx+dy*dy))*a.bias;out.push({region,w});}out.sort((a,b)=>b.w-a.w);const a=out[0],b=out[1],sum=Math.max(.0001,a.w+b.w),secondary=Math.min(.38,(b.w/sum)*.64),bucket=Math.round(secondary*10)/10;return{primary:a.region,secondary:b.region,secondaryAlpha:bucket,key:`${a.region}:${b.region}:${bucket.toFixed(1)}`};}
  ...
  function drawCell(c,material,gx,gy,wx,wy,dx,dy){drawBaseCrop(c,material.primary,gx,gy,dx,dy,1,71);if(material.secondaryAlpha>.01)drawBaseCrop(c,material.secondary,gx,gy,dx,dy,material.secondaryAlpha,173);const semantic=regionAt(wx,wy),row=rows[semantic],s=strength(semantic,wx,wy);if(s>.06){const h2=hash(gx,gy,113),fx=crop(h2),fy=crop(h2>>>5);c.globalAlpha=.16+s*.40;c.drawImage(atlas,256+fx,row*256+fy,128,128,dx,dy,CELL_PX,CELL_PX);c.globalAlpha=1;}const density={village:17,meadow:13,grove:12,fen:14,copper:15,stonepine:13,den:18}[semantic];if(s<.22&&hash(gx,gy,151)%density===0){const size=14+(hash(gx,gy,193)%8),pad=(CELL_PX-size)/2;c.globalAlpha=.46;c.drawImage(atlas,512,row*256,256,256,dx+pad,dy+pad,size,size);c.globalAlpha=1;state.lastDecal=`build47-${semantic}`;}}
  function buildChunk(cx,cy,material){const canvas=document.createElement('canvas');canvas.width=CHUNK_PX;canvas.height=CHUNK_PX;const c=canvas.getContext('2d',{alpha:false}),wx0=cx*CHUNK_WORLD,wy0=cy*CHUNK_WORLD;for(let ty=0;ty<CHUNK_TILES;ty++)for(let tx=0;tx<CHUNK_TILES;tx++){const wx=wx0+(tx+.5)*TILE_WORLD,wy=wy0+(ty+.5)*TILE_WORLD,gx=Math.floor(wx/TILE_WORLD),gy=Math.floor(wy/TILE_WORLD);drawCell(c,material,gx,gy,wx,wy,tx*CELL_PX,ty*CELL_PX);}state.cacheBuilds++;return{canvas,last:++clock};}
  function chunk(cx,cy,material){const key=`${material.key}:${cx},${cy}`;let entry=cache.get(key);if(entry){entry.last=++clock;state.cacheHits++;return entry;}state.cacheMisses++;entry=buildChunk(cx,cy,material);cache.set(key,entry);if(cache.size>CACHE_LIMIT){let victim=null,old=Infinity;for(const[k,v]of cache)if(v.last<old){old=v.last;victim=k;}if(victim){cache.delete(victim);state.evictions++;}}state.activeCache=cache.size;return entry;}
  function enabled(){return Boolean(requested&&state.ready&&!state.failed);}
  function drawOwnedGround(){const material=materialMixAt(camera.x,camera.y);state.materialPrimary=material.primary;state.materialSecondary=material.secondary;state.materialMix=material.secondaryAlpha;ctx.fillStyle=bg[material.primary]||'#465047';ctx.fillRect(0,0,viewport.w,viewport.h);state.frameChunks=0;state.frameCells=0;const centerCx=Math.floor(camera.x/CHUNK_WORLD),centerCy=Math.floor(camera.y/CHUNK_WORLD),unit=PIXEL_WORLD;for(let cy=centerCy-1;cy<=centerCy+1;cy++)for(let cx=centerCx-1;cx<=centerCx+1;cx++){const entry=chunk(cx,cy,material),p=worldToScreen(cx*CHUNK_WORLD,cy*CHUNK_WORLD);ctx.save();ctx.translate(p.x,p.y);ctx.transform(.78*camera.zoom*unit,.39*camera.zoom*unit,-.78*camera.zoom*unit,.39*camera.zoom*unit,0,0);ctx.drawImage(entry.canvas,0,0);ctx.restore();state.frameChunks++;state.frameCells+=64;}}
