(() => {
  ...
  const el=document.createElement('section');el.id='onboarding37-prompt';el.hidden=true;el.setAttribute('aria-label','Warden tutorial');el.innerHTML='<div class="eyebrow">WARDEN TIP</div><b></b><span></span><button id="onboarding37-skip" type="button">Skip tips</button>';document.getElementById('game-shell')?.appendChild(el);
  ...
  el.querySelector('#onboarding37-skip').addEventListener('click',()=>{saved.skipped=true;persist('skipped');forced=null;debug.skipOnboarding?.();render()});
